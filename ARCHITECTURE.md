...
A request is permitted only when every gate that applies to it passes. Ownership is one such gate, an input to that decision rather than a rule beside it. A mutation on a tile's own record passes one gate, the level. A mutation on something owned — a query, a connection, an appearance setting, or an entry inside a tile's state — passes two: the level, and ownership by the acting account. Neither gate grants on its own, so no level is skipped because something is owned, and owning something reaches nothing without a level. A level that reaches what others own — `write` on `data` — passes the ownership gate for every entry, so correcting someone else's entry stays possible for an account trusted with it. All of it falls out of the same call rather than bypassing it.
