...
would drift out of sync with this project's real chart tokens. For where this
tile should sit and how big relative to what's already placed, read
`.claude/agents/dashboard_layout_guidelines.md` — it's written in a
`colSpan`/`rowSpan` grid vocabulary this project doesn't use; translate its
tiering, density-pairing, and aspect-ratio guidance into this project's real
sizes (`sm`/`md`/`lg`, two `md` per row, one `lg` per row), never its literal
column numbers. Then, inside the writable workspace the MCP's instructions
named:
